...
| Date | What was done |
|------|--------------|
| 2026-08-22 | Multi-campus admin foundation + compliance: Campus model + SUPER_ADMIN role (User/Restaurant/Rider gain campusId), Restaurant.approvalStatus (PENDING default) + Rider.isSuspended, super admin can create campuses and assign school admins, campus-scoped admin queries (school admin = own campus, super admin = override), restaurant approve/reject/suspend + rider verify/suspend endpoints, customer restaurant list filters to APPROVED only, suspended riders blocked from accepting orders, client SUPER_ADMIN routing + scope indicator + Campuses/Restaurants/Riders dashboard sections, DIRECT_URL sslmode fix for migrate deploy |
